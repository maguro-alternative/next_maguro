...
//ページネーションの追加
export const Pagination = ({ totalCount,categoryid }:count) => {
  const PER_PAGE = 5;
        ...
        <div className="textback">
          <ul>
            <li className='inline-block relative m-2'>
                <Link  href={ `/${categoryid}/page/1`}>
                  <a className='sm:text-2xl text-sm'>&lt;&lt;</a>
                </Link>
            </li>
            <li className='inline-block relative m-2'>
                <Link  href={ `/${categoryid}/page/${page_back_num}`}>
                  <a className='sm:text-2xl text-sm'>&lt;</a>
                </Link>
            </li>
            {range(1, Math.ceil(totalCount / PER_PAGE)).map((number, index) => (
              <li className='inline-block relative m-2' key={index}>
                <Link  href={ `/${categoryid}/page/${number}`}>
                  <a className='text-2xl'>{number}</a>
                </Link>
              </li>
            ))}
            <li className='inline-block relative m-2'>
                <Link  href={ `/${categoryid}/page/${page_forward_num}`}>
                  <a className='sm:text-2xl text-sm'>&gt;</a>
                </Link>
            </li>
            <li className='inline-block relative m-2'>
                <Link  href={ `/${categoryid}/page/${Math.ceil(totalCount / PER_PAGE)}`}>
                  <a className='sm:text-2xl text-sm'>&gt;&gt;</a>
                </Link>
            </li>
          </ul>
